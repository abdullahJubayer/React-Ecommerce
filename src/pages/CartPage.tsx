...
export const Cartpage: React.FC = () => {
  ...
  return (
    <div>
      ...
      <NavBar />
      <hr className="h-px border-0 bg-gray-200" />
      <div className="flex justify-between my-8 md:my-16 mx-4 md:mx-16">
      ...
      <hr className="h-px border-0 bg-gray-200 mx-4 md:mx-16 mt-2" />
      {cartItems.map((item) => (
        <CartItem key={item.id} item={item} />
      ))}
      <hr className="h-px border-0 bg-gray-200 mx-4 md:mx-16 mt-8" />
      <div className="flex flex-col md:items-end mx-4 md:mx-16">
      ...
      <p className="text-xl text-gray-700 mx-4 md:mx-16 mt-8">
        You may also like
      </p>
      {cartItems.length > 0 && (
